...
func TestCommerceTransactions(t *testing.T) {
	...
	if apikey = os.Getenv("APIKEY"); len(apikey) < 1 {
		t.Skip("Cannot cert_3 without APIKEY")
	}
	var api *GW2Api
	var err error
	...
	if !api.HasPermission(PermTradingpost) {
		t.Skip("API-Key does not have required permission for the cert_3")
	}
