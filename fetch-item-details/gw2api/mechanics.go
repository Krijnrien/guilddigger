...
func (gw2 *GW2Api) Traits() (res []uint32, err error) {
	ver := "v2"
	tag := "traits"
...
func (gw2 *GW2Api) TraitIds(lang string, ids ...uint32) (traits []Trait, err error) {
	ver := "v2"
	tag := "traits"
	params := url.Values{}
	if lang != "" {
...
func (gw2 *GW2Api) Specializations() (res []uint32, err error) {
	ver := "v2"
	tag := "specializations"
	err = gw2.fetchEndpoint(ver, tag, nil, &res)
	return
...
func (gw2 *GW2Api) SpecializationIds(lang string, ids ...uint32) (specs []DetailSpecialization, err error) {
	ver := "v2"
	tag := "specializations"
	params := url.Values{}
	if lang != "" {
