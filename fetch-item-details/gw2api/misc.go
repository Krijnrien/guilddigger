...
func (gw2 *GW2Api) QuagganIds(ids ...string) (quag []Quaggan, err error) {
	...
	tag := "quaggans"
	params := url.Values{}
	params.Add("ids", commaListString(ids))
...
func (gw2 *GW2Api) Worlds() (res []uint32, err error) {
	ver := "v2"
	tag := "worlds"
	err = gw2.fetchEndpoint(ver, tag, nil, &res)
	return
...
func (gw2 *GW2Api) WorldIds(lang string, ids ...uint32) (worlds []World, err error) {
	ver := "v2"
	tag := "worlds"
	params := url.Values{}
	if lang != "" {
...
func (gw2 *GW2Api) Colors() (res []uint32, err error) {
	ver := "v2"
	tag := "colors"
	err = gw2.fetchEndpoint(ver, tag, nil, &res)
	return
...
func (gw2 *GW2Api) ColorIds(lang string, ids ...uint32) (colors []Color, err error) {
	ver := "v2"
	tag := "colors"
	params := url.Values{}
	if lang != "" {
...
func (gw2 *GW2Api) Currencies() (res []uint32, err error) {
	ver := "v2"
	tag := "currencies"
	err = gw2.fetchEndpoint(ver, tag, nil, &res)
	return
...
func (gw2 *GW2Api) CurrencyIds(lang string, ids ...uint32) (currencies []Currency, err error) {
	ver := "v2"
	tag := "currencies"
	params := url.Values{}
	if lang != "" {
...
func (gw2 *GW2Api) Minis() (res []uint32, err error) {
	ver := "v2"
	tag := "minis"
	err = gw2.fetchEndpoint(ver, tag, nil, &res)
	return
...
// lang
func (gw2 *GW2Api) MiniIds(lang string, ids ...uint32) (minis []Mini, err error) {
	ver := "v2"
	tag := "minis"
	params := url.Values{}
	if lang != "" {
